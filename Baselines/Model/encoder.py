...
class TransformerEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.feat_dim = cfg.TransformerEncoder.feat_dim
        ...
        for i in range(len(self.feat_dim)):
            self.project_inp.append(nn.Linear(self.feat_dim[i], self.inter_dim).cuda())
    ...
    def forward(self, input_list):
        ...
        for i in range(len(input_list)):
            input = input_list[i].cuda()
            input = input.permute(1, 0, 2)
            input = self.project_inp[i](input) * math.sqrt(self.inter_dim)
